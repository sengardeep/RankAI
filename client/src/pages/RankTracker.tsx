...
// 1. Fetch Keywords on Component Load
    useEffect(() => {
   ...
   // 2. Handle Adding a New Keyword
    const handleAdd = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.data.success) {
                // Prepend the new tracking keyword
   ...
   // 3. Handle Refreshing a Keyword
    const handleRefresh = async (id: string) => {
        setRefreshing(id);
        try {
            await api.post(`/api/rank/${id}/refresh`);

            // Optimistically update status to 'checking' locally
            setKeywords(prev => prev.map(k => k._id === id ? { ...k, status: 'checking' } : k));

            // Polling for refresh completion
            const pollInterval = setInterval(async () => {
                try {
                    const check = await api.get(`/api/rank/${id}`);
                    if (check.data.tracking.status !== 'checking') {
  ...
  // 4. Handle Deleting a Keyword
    ...
    // 5. Handle Toggling Tracking (Active/Inactive)
    ...
    // Helper functions for JSX (visible in viewport frame)
    const getPositionBadge = (pos: number | null) => {
        if (pos === null) return { text: 'Not Ranked', class: 'text-muted-foreground bg-muted/50' };
        if (pos <= 3) return { text: `#${pos}`, class: 'text-emerald-400 bg-emerald-500/10 border border-emerald-500/20' };
        if (pos <= 10) return { text: `#${pos}`, class: 'text-primary bg-primary/10 border border-primary/20' };
        if (pos <= 20) return { text: `#${pos}`, class: 'text-accent bg-accent/10 border border-accent/20' };
        return { text: `#${pos}`, class: 'text-muted-foreground bg-muted/10 border border-muted/20' };
    };
