...
export default function Analyze() {
    ...
    const handleAnalyze = async (submitUrl?: string) => {
        ...
        setError("");
        setAnalyzing(true);
        setCurrentStep(0); // Step 0: Connecting
        ...
        try {
            ...
            setCurrentStep(1);
             // Step 1: Scanning
            // Call for completion (Polling every 2 seconds)
            let attempt = 0;
            ...
            pollRef.current = setInterval(async () => {
                ...
                try {
                    ...
                    if (pollRef.current) clearInterval(pollRef.current);
                    pollRef.current = null;
                    setError(err?.response?.data?.message || err?.message || "Failed to check analysis status");
                    setAnalyzing(false);
                }
            }, 2000);
